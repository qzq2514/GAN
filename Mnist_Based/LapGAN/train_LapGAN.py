...
#[28*28,14*14,100]或[28*28,100]或[100]即可分别生成三级、两级和一级的LapGAN
...
model_name="LapGAN_fashion_3level"
    ...
    with tf.Session() as sess:
        ...
        for _ in range(epochs):
            for _ in range(step_per_epoch):
                indicex = np.random.choice(mnist.train.num_examples, batch_size)
                batch = (mnist.train.images[indicex], mnist.train.labels[indicex])
                ...
                d_loss_eval = []
                for ind, prior_size in enumerate(prior_scales):
                    #因为最上层使用全连接训练生成图像,而MNIST图像至少也要500个epoch(batch_size=128)
                    #才能达到效果,所以这里对于最后一层要多多训练
                    #这里也是我一直掉坑的地方
                    trian_steps = 1
                    if ind == len(prior_scales)-1:
                        trian_steps=step_per_epoch
                    for _ in range(trian_steps):
                        cur_d_loss, _ = sess.run([d_loss[ind],d_opt[ind]],feed_dict=feed_dict_train)
                        global_step_,cur_g_loss, _ = sess.run([global_step,g_loss[ind], g_opt[ind]],feed_dict=feed_dict_train)

                    d_loss_eval.append(cur_d_loss)
                    g_loss_eval.append(cur_g_loss)

                step_per_step = (step_per_epoch+len(prior_scales)-1)

                epoche_num = (global_step_/step_per_step )// step_per_epoch
                cur_epoch_step = int((global_step_ / step_per_step) % step_per_epoch)
                print("Epoch:{},Global_step(pyramid):{}/{} time:{}ms\nD_Loss: {} \nG_Loss: {}".format(
                    epoche_num,cur_epoch_step,step_per_epoch, time.time() - start_time, d_loss_eval, g_loss_eval))
                ...
                step_pyramid_=epoche_num*step_per_epoch+cur_epoch_step
                if step_pyramid_ % snapshot == 0:
                    ...
                    for scale_index in range(len(prior_scales)):
                        ...
                        for index in range(10):
                            ...
                            save_image_path = "{}/generated_img_{}_{}_{}.jpg".\
                                        format(save_dir, batch[1][index], step_pyramid_,index)
                            # print("---------------",save_image_path)
                            cv2.imwrite(save_image_path, cur_generated_out[index])
                        ...
                        if scale_index!=len(prior_scales):
                            ...
                            fine_sample = ((corase_img_out + 1) / 2) * 255.0
                        ...
                        for index in range(10):
                            cv2.imwrite("{}/corase_img_{}_{}_{}.jpg".
                                        format(save_dir,batch[1][index], step_pyramid_,index), corase_img_out[index])
                            cv2.imwrite("{}/sr_sample_{}_{}_{}.jpg".
                                        format(save_dir,batch[1][index], step_pyramid_,index), fine_sample[index])
                            cv2.imwrite("{}/real_sample_{}_{}_{}.jpg".
                                        format(save_dir,batch[1][index], step_pyramid_,index), real_image[index])

                    # 保存PB
                    constant_graph = graph_util.convert_variables_to_constants(sess,
                                                    sess.graph_def, generated_out_names) #
                    save_model_name = model_name + "-" + str(step_pyramid_) + ".pb"
                    with tf.gfile.FastGFile(pb_path + save_model_name, mode="wb") as fw:
                        fw.write(constant_graph.SerializeToString())
                    # 保存CKPT
                    saver.save(sess, ckpt_path + model_name + ".ckpt", global_step=step_pyramid_)
