...
#噪声为向量形式,且各个生成器之间独立训练、独立生成,就相当于多个cGAN
#每个独立的GAN使用图形高斯金字塔对应的图像作为真实图像进行判别
class LapGAN_mnist:
    ...
    def generator(self,coarse,z_prior,label,generator_ind,reuse=False):
        ...
        with tf.variable_scope("generator_{}_{}".format(generator_ind,scope_name),reuse=reuse):
    ...
    def discriminator(self,res_image,coarse,label,discriminator_ind,reuse=False):
        ...
        with tf.variable_scope("discriminator_{}_{}".format(discriminator_ind,scope_name), reuse=reuse):
    ...
    def build_LapGAN(self):
        ...
        for ind in range(self.pyramid_levels):
            ...
            #self.discriminator(lap_img,coarse_img,....
            d_real_prob,d_real_logits=self.discriminator(gauss_img,coarse_img,self.label_placeholder,
                                                         ind,reuse=True)
    ...
    def get_vars(self):
        ...
        for ind in range(self.pyramid_levels):
            ...
            cur_g_vars = [var for var in all_vars if var.name.startswith("generator_{}_{}".
                                                                         format(ind,scope_name))]
            cur_d_vars = [var for var in all_vars if var.name.startswith("discriminator_{}_{}".
                                                                         format(ind,scope_name))]
