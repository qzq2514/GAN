...
from net import LapGAN_mnist_dependent_noConv as LapGAN_mnist
import numpy as np
...
prior_scales=[100]   #写成这样,那么GAN就直接退化成一个cGAN
# prior_scales=[28*28, 14*14, 100]
...
model_name="LapGAN_mnist_oneLevel"
...
def train():
    ...
    all_g_vars=[]
    for ind,prior_size in enumerate(prior_scales):
        g_opt.append(tf.train.AdamOptimizer(learning_rate).minimize(g_loss[ind], var_list = g_vars[ind]))
        d_opt.append(tf.train.AdamOptimizer(learning_rate).minimize(d_loss[ind], var_list = d_vars[ind]))
