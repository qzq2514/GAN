...
class LSGAN_mnist:
    def __init__(self,smooth,is_training,num_class,batch_size,image_width,image_height,image_channal):
        self.smooth=smooth
    ...
    def discriminator(self,inputs,label,reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            conv3 = tf.layers.conv2d(lrelu2, 1, [7, 7], strides=(1, 1), padding='valid',
    ...
    def loss(self, y_data, y_generated):
        d_loss = tf.reduce_mean(0.5 * tf.square(y_data - 1) + 0.5 * tf.square(y_generated))
        g_loss = tf.reduce_mean(0.5 * tf.square(y_generated - 1))
        return g_loss, d_loss

    def loss_CE(self,y_data,y_generated):
