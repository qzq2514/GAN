...
model_name="InfoGAN_fashion_38noise_"
...
def train():
    ...
    latent_code_placeholder = tf.placeholder(tf.float32, shape=[None,laten_code_size], name="latent_code")
    is_training_placeholder = tf.placeholder_with_default(False,shape=(),name="is_training")

    mnist_GANer=InfoGAN_mnist.InfoGAN_mnist(smooth,is_training_placeholder,
                                            laten_code_size,num_class,batch_size)

    generated_data = mnist_GANer.generator(z_prior_placeholder,latent_code_placeholder,label_placeholder)
    print("generated_data:",generated_data)
    _generated_data = tf.identity(generated_data,name="generated_output")

    d_sigmoid_real, q_cat_logit_real, _ = \
         mnist_GANer.discriminator(real_data_placeholder,label_placeholder)
    d_sigmoid_fake, q_cat_logit_fake, q_latent_sigmoid_fake = \
    ...
    g_loss, d_loss = mnist_GANer.loss(d_sigmoid_real, d_sigmoid_fake,label_placeholder,
                                      q_cat_logit_real,q_cat_logit_fake,latent_code_placeholder,
                                      q_latent_sigmoid_fake)
    ...
    saver = tf.train.Saver()   #为了为了能够接着之前的训练继续训练,所以这里最好不要只保存生成器的参数
    ...
    with tf.Session() as sess:
        ...
        for epoch_num in range(epochs):
            ...
            for _ in range(mnist.train.num_examples//batch_size):
                ...
                batch_laten_code = np.random.normal(0, 1, size=(batch_size, laten_code_size))

                #更新判别器
                feed_dict={real_data_placeholder:batch_images,
                                    z_prior_placeholder:batch_z_prior,
                                    label_placeholder:batch_labels,
                                    latent_code_placeholder:batch_laten_code,
                ...
                if global_step_ % snapshot == 0:
                    # 显示中间结果
                    samples = sess.run(generated_data, feed_dict=feed_dict)
