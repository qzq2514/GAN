...
class ACGAN_mnist:
    ...
    #这里即为ACGAN的特点,ACGAN带类别条件但是判别器是不接收类条件信息的,
    #而之前的带类别条件的DCGAN的判别器是带类条件信息的
    def discriminator(self,inputs,reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            w_init = tf.truncated_normal_initializer(mean=0.0, stddev=0.02)
            b_init = tf.constant_initializer(0.0)

            conv1 = tf.layers.conv2d(inputs, 64, [4, 4], strides=[2, 2], padding="same",
                                                 kernel_initializer=w_init, bias_initializer=b_init)
    ...
    def loss(self,prob_real,prob_fake,real_label,cat_logit_real,cat_logit_fake):
        ...
        ##ACGAN的核心:使用类别信息辅助更新判别器,保证在增强判别器的同时可以“监督”生成更真实的图片
