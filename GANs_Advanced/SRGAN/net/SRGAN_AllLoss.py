...
class SRGAN:
    ...
    def get_content_loss(self,feature_a,feature_b):
        print("Using summmary of VGG loss and MSE loss as content loss!")
        ...
        image_mse_loss=tf.reduce_mean(tf.losses.absolute_difference(feature_a , feature_b))

        return image_mse_loss,vgg_loss
    ...
    def discriminator(self,inputs,name_scope,reuse=False):
        print("SRGAN_AllLoss_discriminator")
        with tf.variable_scope(name_scope,reuse=reuse) as scope:
            w_init = tf.truncated_normal_initializer(mean=0.0, stddev=0.02)
            with slim.arg_scope([slim.conv2d], weights_initializer=w_init, padding="SAME", activation_fn=None):
                with slim.arg_scope([slim.conv2d_transpose], weights_initializer=w_init, padding="SAME",activation_fn=None):
    ...
    def build_CartoonGAN(self,LR,HR):
        ...
        image_mse_loss,vgg_loss = self.get_content_loss(HR_pre, fake_HR)
        
        gen_loss = fake_gen_loss + self.vgg_weight*(image_mse_loss+2e-6*vgg_loss)

        return gen_loss,dis_loss,image_mse_loss,psnr
